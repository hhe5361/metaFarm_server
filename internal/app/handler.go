...
func analysisHandler(storage storage.Storage, openaiKey string) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		go func() {
			...
			{
				"name": "Korean name of the plant (prioritize exact matching to one of these: 토마토, 딸기, 호박, 당근, 옥수수, 고추. For example, '로메인 상추' should be '상추', but '수박' should not match to any of these. Only use other names if none of these match exactly)",
